...
if [ -n "${COMPOSE_FILE:-}" ]; then
  ...
  upsert_env_value "$ENV_PATH" "GOVPRESS_WORKER_CONCURRENCY" "${GOVPRESS_WORKER_CONCURRENCY:-4}"
  ...
  upsert_env_value "$ENV_PATH" "GOVPRESS_MAX_UPLOAD_BYTES" "25000000"
  upsert_env_value "$ENV_PATH" "GOVPRESS_HWPX_MD_TIMEOUT_SECONDS" "600"
  upsert_env_value "$ENV_PATH" "GOVPRESS_WORKER_CONCURRENCY" "${GOVPRESS_WORKER_CONCURRENCY:-1}"
  upsert_env_value "$ENV_PATH" "GOVPRESS_LARGE_WORKER_CONCURRENCY" "${GOVPRESS_LARGE_WORKER_CONCURRENCY:-0}"
  upsert_env_value "$ENV_PATH" "GOVPRESS_WORKER_MEM_LIMIT" "${GOVPRESS_WORKER_MEM_LIMIT:-512m}"
  upsert_env_value "$ENV_PATH" "GOVPRESS_CONVERTER_SPEC" "$CONVERTER_SPEC"
  upsert_env_value "$ENV_PATH" "GOVPRESS_CONVERTER_ALLOW_LOCAL_FALLBACK" "0"
  upsert_env_value "$ENV_PATH" "GOVPRESS_HWPX_MD_SPEC" "${HWPX_MD_SPEC:-}"
