...
python3 "$GOV_MD_CONVERTER_ROOT/scripts/evaluate_policy_briefing_qc_report.py" \
  "$REPORT_PATH" \
  --summary-markdown "$SUMMARY_PATH"

python3 "$GOV_MD_CONVERTER_ROOT/scripts/build_policy_briefing_qc_dashboard.py" \
  --root "$OUTPUT_ROOT" \
