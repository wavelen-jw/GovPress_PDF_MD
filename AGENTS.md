...
- For frontend changes, run `npm run typecheck` in `mobile/` when TypeScript or component code changes. Reuse the existing `mobile/node_modules` install; do not run `npm ci` unless dependencies are missing or `package-lock.json` changed.
