...
3. `.venv`가 없거나 깨져 있으면 재생성 후 의존성 설치
...
- `GOVPRESS_VPS_SSH_KEY`
  - 권장: OpenSSH 개인키 전체 내용
  - 대안: 개인키를 base64 인코딩한 문자열도 허용
...
- GitHub Secrets에서 줄바꿈이 깨지는 환경이면 아래처럼 base64로 넣을 수 있습니다.

```bash
base64 -w0 ~/.ssh/govpress_actions
```
